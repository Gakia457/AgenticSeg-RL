...
def calculate_metrics(output_dir):
    ...
    if not output_files:
        _log(f"cannot find output files in {output_dir}")
        return
    ...
    # print the results
    _log(f"Processed {n_items} samples.")
    _log(f"gIoU (average of per image IoU): {gIoU:.4f}")
    _log(f"cIoU (sum of intersections / sum of unions): {cIoU:.4f}")
    if resp_len_count > 0:
        avg_rl = total_resp_len / float(resp_len_count)
        _log(f"Avg response length (tokens): {avg_rl:.2f}")
    else:
        _log("Avg response length (tokens): N/A (no 'response_length' in outputs)")
    ...
    if len(type_stats) > 0:
        ...
        for rt in sorted(type_stats.keys()):
            ...
            _log(f"[reasoning_type={rt}] samples={cnt} gIoU={g:.4f} cIoU={c:.4f}")
    ...
    if len(sc_turn_count) > 0:
        for t in sorted(sc_turn_count.keys()):
            ...
            _log(f"[Self-correction@turn={t}] Processed {cnt} samples.")
            _log(f"[Self-correction@turn={t}] gIoU (average of per image IoU): {sc_gIoU:.4f}")
            _log(f"[Self-correction@turn={t}] cIoU (sum of intersections / sum of unions): {sc_cIoU:.4f}")
            _log(f"[Self-correction@turn={t}] mean(+ΔIoU): {mean_pos:.6f}")
            _log(f"[Self-correction@turn={t}] mean(-ΔIoU): {mean_neg:.6f}")
            _log(f"[Self-correction@turn={t}] mean(ΔIoU): {mean_delta:.6f}  (should equal sc_gIoU - base_gIoU_on_sc = {(sc_gIoU - base_gIoU_on_sc):.6f})")

    txt_path = os.path.join(output_dir, "calculate_iou_summary.txt")
