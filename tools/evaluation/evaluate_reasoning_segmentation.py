...
def parse_args():
    ...
    parser.add_argument("--reasoning_model_path", type=str, default="pretrained_models/reasoning-model")
    ...
    parser.add_argument("--dump_analysis_fields", type=str2bool, default=False,
                        help="If true, write extra per-sample fields (query/pred coords/areas/vis path) for case analysis.")
...
def cluster_mask_candidates(mask_candidates: List[MaskCandidate],
    ...
    for idx, cand in enumerate(mask_candidates):
        ...
        for cluster in clusters:
            rep_idx = cluster["member_indices"][0] # 取该簇的第一个成员作为代表
...
def main():
    ...
    def _safe_list_of_int_list(v):
        if not isinstance(v, (list, tuple)):
            return []
        out = []
        for one in v:
            casted = _safe_int_list(one)
    ...
    def _attach_analysis_fields(
        ...
        *,
        ...
        if not bool(args.dump_analysis_fields):
            return

        out_item["query"] = str(meta.get("text", ""))
        out_item["has_bbox_annotation"] = bool(meta.get("bbox") is not None)
        if meta.get("bbox") is not None:
            gt_bbox = _safe_int_list(meta.get("bbox"))
            if gt_bbox is not None:
                out_item["gt_bbox"] = gt_bbox

        out_item["pred_bboxes"] = _safe_list_of_int_list(pred_bboxes if pred_bboxes is not None else [])
        out_item["pred_points"] = _safe_list_of_int_list(pred_points if pred_points is not None else [])
        out_item["pred_labels"] = _safe_str_list(pred_labels)

        try:
            gt_mask = np.array(meta.get("mask")).astype(bool)
            out_item["gt_mask_area"] = int(gt_mask.sum())
        except Exception:
            pass

        if pred_mask is not None:
            try:
                out_item["pred_mask_area"] = int(np.array(pred_mask).astype(bool).sum())
            except Exception:
                pass

        inter = out_item.get("intersection", None)
        uni = out_item.get("union", None)
        if isinstance(inter, (int, float)) and isinstance(uni, (int, float)):
            out_item["iou"] = (float(inter) / float(uni)) if float(uni) > 0 else 1.0

        if vis_sample_dir:
            try:
                out_item["vis_relpath"] = os.path.relpath(vis_sample_dir, args.output_path)
            except Exception:
                pass
            ...
            with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                for id_idx in range(len(batch_output_text)):
                    ...
                    try:
                        ...
                        _attach_analysis_fields(
                            out_item,
                            meta,
                            pred_bboxes=[],
                            pred_points=[],
                            pred_labels=[],
                            pred_mask=np.zeros_like(np.array(meta["mask"]).astype(bool)),
                            vis_sample_dir=sample_dir,
                        )
                    ...
                    _attach_reasoning_type(out_item, meta)
                    final_pred_bboxes = list(bboxes)
                    final_pred_points = list(points)
                    final_pred_labels = list(label) if isinstance(label, list) else ([] if label is None else [label])
                    final_pred_mask = np.array(mask_all).astype(bool)
                    ...
                    if args.use_self_correction and (not args.use_majority_voting) and int(args.sc_num_turns) > 0:
                        ...
                        for t in range(1, sc_turns + 1):
                            ...
                            final_pred_bboxes = list(final_bboxes)
                            final_pred_points = list(final_points)
                            final_pred_labels = list(final_labels)
                            final_pred_mask = np.array(sc_mask_all).astype(bool)

                    _attach_analysis_fields(
                        out_item,
                        meta,
                        pred_bboxes=final_pred_bboxes,
                        pred_points=final_pred_points,
            ...
            with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
                for id_idx in range(batch_size):
                    ...
                    if args.use_self_correction and int(args.sc_num_turns) > 0:
                        ...
                        # MaskCandidate 的收集逻辑
                        # 文本输出提取bbox/point/label信息 - 处理空对象 - SAM2预测
                        # 输出 bboxes/points/labels/thinks/masks
                        def _accumulate_from_text(out_text: str, run_sample_idx: int):
                            ...
                            try:
                                # 对每次采样提取文本答案中的bbox/point/label信息
                        ...
                        _attach_analysis_fields(
                            ...
                            pred_bboxes=[],
                            pred_points=[],
                            pred_labels=final_labels if isinstance(final_labels, list) else [],
                            pred_mask=np.array(final_mask_all).astype(bool),
                            vis_sample_dir=None,
                        )
                    ...
                    mv_pred_bboxes = []
                    mv_pred_points = []
                    for fm in (final_masks_for_viz or []):
                        try:
                            bb, pt = _mask_to_bbox_point(np.array(fm).astype(bool))
                            mv_pred_bboxes.append(bb)
                            mv_pred_points.append(pt)
                        except Exception:
                            continue
                    _attach_analysis_fields(
                        out_item,
                        meta,
                        pred_bboxes=mv_pred_bboxes,
                        pred_points=mv_pred_points,
                        pred_labels=final_labels if isinstance(final_labels, list) else [],
                        pred_mask=np.array(final_mask_all).astype(bool),
                        vis_sample_dir=sample_dir,
                    )
