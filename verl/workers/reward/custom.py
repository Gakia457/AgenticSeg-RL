...
class CustomRewardManager:
    def __init__(self,
        ...
        self.is_stage2 = (compute_score == "agentic_seg_s2")
        ...
        elif compute_score.startswith("agentic_seg"):
            print("compute score: agentic segmentation")
            self.sam_actor = sam_actor
        else:
            raise NotImplementedError()

    def __call__(self, data: DataProto) -> torch.Tensor:
        # 多卡训练时每个 rank 都会执行到这里；仅 rank0 打印，避免日志重复刷屏。
        rank = int(os.getenv("RANK", "0"))
        should_print = rank == 0
        # [AgenticRL日志] Task3 polygon 很长，只截断控制台打印，不改真实 reward 输入。
        def _short_log_text(value, max_chars: int = 200):
            text = str(value)
        ...
        if is_multi_turn_stage2:
            ...
            for i in range(len(data)):
                ...
                if should_print and already_print < self.num_examine:
                    already_print += 1
        ...
        # ---------- 2) AGENTIC-SEG 계열: SAM2 + multi-GPU DP_COMPUTE_PROTO ----------
        reward_tensor = torch.zeros_like(data.batch["responses"], dtype=torch.float32)
        reward_details = defaultdict(list)
        ...
        for i in range(len(data)): 
            ...
            # [AgenticRL日志-备用] 如需排查 <think> 是否被 skip_special_tokens 过滤，可临时打开：
            # response_str_raw = self.tokenizer.decode(valid_response_ids, skip_special_tokens=False)
            ...
            if should_print and already_print < self.num_examine:
                ...
                # if response_str_raw != response_str:
                #     print("[response_raw]", _short_log_text(response_str_raw))
                print("[ground_truth]", _short_log_text(ground_truth))
        ...
        sam_input = DataProto(
            ...
            meta_info={
                ...
                "compute_score": self.compute_score_type,
            },
        )
        ...
        ious = np.array(reward_details.get("mask_iou", []))
        if len(ious) > 0:
            corrects = (ious >= 2.0).astype(float)
            per_sample_matrix = corrects.reshape(-1, self.rollout_n)
            per_sample_acc = per_sample_matrix.mean(axis=1)
            ...
            if should_print:
                print(f"[Sample-wise Rollout Acc] []")

        reward_stats = {k: sum(v) / len(v) for k, v in reward_details.items() if k != "pred_masks" and len(v) > 0}
