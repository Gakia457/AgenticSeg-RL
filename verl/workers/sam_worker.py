...
from verl.utils.reward_score.agentic_seg import SAM_MODEL_CFG, SAM_CHECKPOINT
...
class SAMPredictorWorker(Worker):
    ...
    def init_model(self):
        # SAM2 在每个 worker/rank 都会触发 INFO 日志；统一降到 WARNING，减少重复刷屏。
    ...
    def _compute_reward_single(
        ...
        """단일 샘플 AGENTIC-SEG reward 계산 로직."""
        # --- [AgenticRL] 路由分发到新任务奖励逻辑 ---
        from verl.agentic.router import route_and_compute_reward
        agentic_score, agentic_details, is_agentic = route_and_compute_reward(
            predict_str, ground_truth, ground_truth_masks, image, self.predictor, is_qwen3=is_qwen3
        )
        if is_agentic:
            return agentic_score, agentic_details
        # ------------------------------------------
        ...
        if not is_stage2:  # 1st stage training
            ...
            accuracy_reward, mask_score, pred_masks = agentic_seg_accuracy_reward(
                predict_str,
                ground_truth,
                ground_truth_masks,
                image,
    ...
    def compute_reward_batch(self, data: DataProto) -> DataProto:
        ...
        # 预填充所有可能的指标键，确保DP一致性。
        possible_keys = [
            "format", "accuracy", "bbox_iou", "bbox_l1", 
            "point_l1", "mask_iou", "count_match", "non_repeat"
        ]
        detail_lists: Dict[str, list] = {k: [] for k in possible_keys}
        ...
        for i in range(num):
            ...
            # Fill known keys, fallback to 0.0
            for k in possible_keys:
                detail_lists[k].append(details.get(k, 0.0))
            
            # For any unexpected dynamically added keys
            for k, v in details.items():
                if k not in possible_keys:
                    if k not in detail_lists:
                        detail_lists[k] = [0.0] * i
...
# 准确率奖励 (基于 bbox IoU, L1 距离, point 距离/包含关系, mask IoU, 以及数量匹配)
def agentic_seg_accuracy_reward(
    predict_str: str,
    ground_truth: str,
        ...
        try:
            ...
            if json_match:
                ...
                if ground_truth_masks is not None and image is not None and len(pred_bboxes) > 0:
                    ...
                    # 参数会传入 SAM2ImagePredictor 的 predict() 进行预测
    ...
    try:
        ...
        if json_match:
            ...
            if ground_truth_masks is not None and image is not None and len(pred_bboxes) > 0:
                ...
                # 掩码阶段式奖励（仅当 bbox IoU > 0.5 且 point 距离 < 30 且点在框内 时才计算），分段奖励如下：
                mask_reward = np.zeros_like(mask_iou)
                mask_reward[(mask_iou > 0.3) & (mask_iou <= 0.5)] = 1.0
                mask_reward[(mask_iou > 0.5) & (mask_iou <= 0.7)] = 2.0
                mask_reward[(mask_iou > 0.7) & (mask_iou <= 0.8)] = 3.0
