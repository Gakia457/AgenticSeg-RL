...
@dataclass
class AgenticVizConfig:
    # [AgenticRL可视化] 这些字段只控制 rollout 可视化，默认关闭，不影响原训练。
    enabled: bool = False
    task: str = "auto"
    log_every_steps: int = 10
    max_samples: int = 1
    max_rollouts_per_sample: int = 8
    seed: int = 42
    save_local: bool = True
    log_to_wandb: bool = True
    output_dir: str = "agentic_rollouts"
...
class TrainerConfig:
    ...
    # [AgenticRL可视化] 命令行可用 trainer.agentic_viz.* 打开；不传时保持旧行为。
    agentic_viz: AgenticVizConfig = field(default_factory=AgenticVizConfig)
