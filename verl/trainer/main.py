...
def main_task(config: PPOConfig):
    ...
    reward_fn = CustomRewardManager(                          # 构造奖励函数
        tokenizer=tokenizer,
        # 不打印[prompt]/[response]/[GT]了  设置为几就打印几条
        num_examine=1,    # num_examine=0,
        compute_score=config.worker.reward.compute_score,     # 在这里可以配置 奖励计算方式
        rollout_n=config.worker.rollout.n,
        sam_actor=None,                                       # L116有补丁 如果compute_score=agentic_seg的话会把sam_actor传进来
        is_qwen3=is_qwen3,
    ...
    if config.worker.reward.compute_score.startswith("agentic_seg"):
